...
# Automatically shut down containers if they are already running before setting up
if [ -n "$COMPOSE_ARGS" ]; then
  RUNNING_SERVICES=$(docker compose $COMPOSE_ARGS ps --services --filter "status=running" 2>/dev/null || true)
  if [ -n "$RUNNING_SERVICES" ]; then
    echo -e "${YELLOW}Detected active container stack running. Automatically shutting down containers before setting up...${NC}"
